...
public class BuyGoldRepository implements AnonymousInterface<BuyGold> {
    ...
    public Receipt receiptData(int id) {
        ...
        try {
            String query = "SELECT gold.*,payment.amount_paid,payment.balance FROM " + BuyGoldDTO.getBUY_GOLD_DB() + " gold "
                    + "LEFT JOIN " + PaymentDTO.getPAYMENT_DB() + " payment ON payment.buy_gold_id=gold.id "
                    + "WHERE gold.id= '" + id + "'";
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();
    ...
    public List<BuyGold> findBuyGoldBetweenDates(String startDate, String endDate) {
        List<BuyGold> buyGoldsList = new ArrayList<>();
        try {
            String query = "SELECT gold.*,user.fullname AS user,customer.fullname AS customer FROM " + BuyGoldDTO.getBUY_GOLD_DB() + " gold "
                    + "LEFT JOIN " + UserDTO.getUSERS_DB() + " user ON gold.user_id=user.id "
                    + "LEFT JOIN " + CustomerDTO.getCUSTOMER_DB() + " customer ON gold.customer_id=customer.id "
                    + "WHERE gold.created_date >= '" + startDate + "' AND gold.created_date <= '" + endDate + "' ORDER BY gold.id DESC";
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();
    ...
    public double goldBuySummation(String createdDate) {
        double totalGoldBought = 0;
        try {
            String query = "SELECT SUM(" + BuyGoldDTO.getTOTAL_AMOUNT() + ") AS total FROM " + BuyGoldDTO.getBUY_GOLD_DB() + " WHERE " + BuyGoldDTO.getRAW_DATE() + " = '" + createdDate + "'";
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();

            if (rs.next()) {
                totalGoldBought = rs.getDouble("total");
            }
    ...
    public double buyGoldSummationBetweenDates(String startDate, String endDate) {
        double totalGoldBought = 0;
        try {
            String query = "SELECT SUM(" + BuyGoldDTO.getTOTAL_AMOUNT() + ") AS total FROM " + BuyGoldDTO.getBUY_GOLD_DB() + " "
                    + "WHERE " + BuyGoldDTO.getRAW_DATE() + " >= '" + startDate + "' AND " + BuyGoldDTO.getRAW_DATE() + " <= '" + endDate + "'";
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();
